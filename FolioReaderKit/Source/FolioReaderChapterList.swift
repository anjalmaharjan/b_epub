...
class FolioReaderChapterList: UITableViewController {
	...
	override func viewDidLoad() {
		...
		self.tableView.contentInset = UIEdgeInsets(top: 20, left: 0, bottom: 0, right: 0) // Adjust top padding here
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		cell.indexLabel?.text = tocReference.title.trimmingCharacters(in: .whitespacesAndNewlines)
			...
			// Add audio duration for Media Overlay
		...
		if
			...
			if tocReference.resource == resource {
					// Selected chapter: Set bold font and selected text color
				cell.indexLabel?.textColor = self.readerConfig.menuTextColorSelected
				cell.indexLabel?.font = UIFont.boldSystemFont(ofSize: cell.indexLabel?.font.pointSize ?? 17)
			} else {
					// Non-selected chapter: Set regular font and normal text color
				cell.indexLabel?.textColor = self.readerConfig.menuTextColor
				cell.indexLabel?.font = UIFont.systemFont(ofSize: cell.indexLabel?.font.pointSize ?? 17)
			}
		}
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		// MARK: - Table view row height customization
	
		// Custom height for first row
	override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
			// Check if it's the first row (index 0)
		if indexPath.row == 0 {
			return 50 // Custom height for the first row (you can adjust this value)
		}
			// Return the default height for other rows
